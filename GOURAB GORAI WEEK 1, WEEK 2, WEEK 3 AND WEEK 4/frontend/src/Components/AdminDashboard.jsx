        ...
        const fetchAdminData = async () => {
            ...
            try {
                ...
                if (settingsRes.ok) {
                    ...
                    if (settingsData.feed_visible !== undefined) {
                        setFeedVisible(settingsData.feed_visible === 'true');
                    }
                ...
                <div className="bg-white/70 backdrop-blur-xl border border-white/50 shadow-2xl rounded-3xl overflow-hidden mb-8">
                    ...
                    <div className="p-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <h3 className="text-lg font-semibold text-gray-800">Shoutout Feed Visibility</h3>
                                <p className="text-gray-600 text-sm mt-1">
                                    {feedVisible
                                        ? "Feed is currently visible to all employees."
                                        : "Feed is currently HIDDEN. New posts will be stored but not displayed."}
                                </p>
                            </div>

                            <button
                                onClick={() => toggleSetting('feed_visible', feedVisible, setFeedVisible)}
                                className={`
                                    ...
                                    ${feedVisible ? 'bg-purple-600' : 'bg-gray-400'}
                                `}
                            >
                                <span className="sr-only">Toggle Feed Visibility</span>
                                <span
                                    className={`
                                        inline-block w-6 h-6 transform bg-white rounded-full transition-transform duration-200 shadow
                                        ${feedVisible ? 'translate-x-8' : 'translate-x-0'}
                                    `}
                                />
                            </button>
                        </div>
