...
export default function ShoutoutFeed({ currentUserId }) {
    ...
    const getRecencyBadge = (isoString) => {
        ...
        const diff = (new Date() - new Date(isoString)) / 1000 / 60; // diff in minutes
    ...
    return (
        <div className="w-full max-w-6xl mx-auto mt-10 pb-20 space-y-12 px-4">
            ...
            <div className="space-y-10">
                {feed.map((post) => (
                    <div key={post.id} className="bg-white/5 backdrop-blur-xl p-10 rounded-[4rem] border border-white/10 shadow-2xl relative">
