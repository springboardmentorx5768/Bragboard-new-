...
def login(request: schemas.UserLogin, db: Session = Depends(get_db)):
    ...
    user = db.query(models.User).filter(
        (models.User.email == request.email) | (models.User.user_id == request.email)
    ).first()
    ...
    # For employee type, we allow defaults (both admin and employee can login, or restrict if needed)
    # The requirement said "only allow admin to enter with his email and password" for admin section.
