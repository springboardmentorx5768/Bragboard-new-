...
def create_comment(
    ...
    # Ensure created_at is set if database default didn't work immediately
...
def delete_comment(
    ...
    if current_user.role == models.UserRole.admin:
        db_log = models.AdminLog(
            ...
            action=f"Deleted comment #{comment_id} on shoutout #{id}",
...
def delete_shoutout(
    ...
    if current_user.role == models.UserRole.admin:
        db_log = models.AdminLog(
            admin_id=current_user.id,
            action=f"Nuked/Deleted shoutout #{id}",
            target_id=id,
            target_type="shoutout"
        )
        db.add(db_log)

    # Explicitly Delete associated reports (in case CASCADE isn't active in DB schema)
    db.query(models.Report).filter(models.Report.shoutout_id == id).delete()
    
    # Also delete comments (if they are not cascaded)
    db.query(models.Comment).filter(models.Comment.shoutout_id == id).delete()
    
    db.delete(shoutout)
    db.commit()

@router.put("/{id}", response_model=schemas.ShoutOutResponse)
def update_shoutout(
    id: int,
    shoutout_update: schemas.ShoutOutCreate,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(auth.get_current_user)
):
    shoutout = db.query(models.ShoutOut).filter(models.ShoutOut.id == id).first()
    ...
    shoutout.title = shoutout_update.title
    shoutout.message = shoutout_update.message
    shoutout.image_url = shoutout_update.image_url
    
    # Update tags if provided
    if shoutout_update.tags is not None:
        shoutout.tags = shoutout_update.tags
    ...
    if current_user.role == models.UserRole.admin:
        db_log = models.AdminLog(
            admin_id=current_user.id,
            action=f"Moderated/Updated shoutout #{id}",
            target_id=id,
            target_type="shoutout"
        )
        db.add(db_log)
    ...
    return enrich_shoutout(shoutout, current_user.id)

@router.post("/{id}/report", response_model=schemas.ReportResponse)
def create_report(
    id: int,
    ...
    # Notify Admins (Optional enhancement, but good for "system")
