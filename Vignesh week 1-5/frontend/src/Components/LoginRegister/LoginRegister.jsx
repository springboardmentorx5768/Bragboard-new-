...
import { FaUser, FaLock, FaEnvelope, FaSignInAlt, FaBriefcase, FaPaperPlane, FaThLarge, FaUsers, FaFilter, FaCheckCircle } from "react-icons/fa"; 
                        ...
                        {view === "dashboard" && (
                            <>
                                ...
                                <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', padding: '15px', background: 'rgba(79, 70, 229, 0.1)', borderRadius: '12px', border: '1px solid #4F46E5', flexWrap: 'wrap', alignItems: 'center' }}>
                                    <span style={{ color: '#484452', fontSize: '0.8rem', fontWeight: 'bold' }}><FaFilter /> FILTERS:</span>
                                    <select style={{ background: '#0D264E', color: 'white', border: '1px solid #434349', borderRadius: '4px', padding: '5px' }} value={filterDept} onChange={(e) => setFilterDept(e.target.value)}>
                                    ...
                                    <input type="date" style={{ background: '#0D264E', color: 'white', border: '1px solid #51444b', borderRadius: '4px', padding: '5px' }} value={filterDate} onChange={(e) => setFilterDate(e.target.value)} />
                                    ...
                                    shoutouts.map(s => (
                                        <div key={s.id} className="shoutout-item" style={{ padding: '20px', marginBottom: '20px' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                                                <div style={{ color: '#d7d4de', fontWeight: 'bold', fontSize: '0.85rem' }}>From: {s.sender_name} (ID: {s.sender_id})</div>
                                                <div style={{ background: '#4f4e64', color: 'white', padding: '2px 10px', borderRadius: '20px', fontSize: '0.65rem', fontWeight: 'bold', border: '1px solid #605a6d' }}>RECOGNIZING: {s.target_dept?.toUpperCase()}</div>
                                            </div>
                                            ...
                                            <div style={{ display: 'flex', gap: '15px', marginTop: '15px', paddingTop: '10px', borderTop: '1px solid #ffffff11' }}>
                                                <button 
                                                    ...
                                                    style={reactBtnStyle}
                                                    onMouseOver={(e) => {e.currentTarget.style.background = '#545360'; e.currentTarget.style.boxShadow = '0 0 10px #424146';}}
                                                    onMouseOut={(e) => {e.currentTarget.style.background = 'rgba(255, 255, 255, 0.05)'; e.currentTarget.style.boxShadow = 'none';}}
                                                >
                                                    👍 <span>{s.reaction_counts?.like || 0}</span>
                                                </button>
                                                <button 
                                                    className="react-btn"
                                                    onClick={() => handleReaction(s.id, 'clap')} 
                                                    style={reactBtnStyle}
                                                    onMouseOver={(e) => {e.currentTarget.style.background = '#4F46E5'; e.currentTarget.style.boxShadow = '0 0 10px #8B5CF6';}}
                                                    onMouseOut={(e) => {e.currentTarget.style.background = 'rgba(255, 255, 255, 0.05)'; e.currentTarget.style.boxShadow = 'none';}}
                                                >
                                                ...
                                                <button 
                                                    className="react-btn"
                                                    onClick={() => handleReaction(s.id, 'star')} 
                                                    style={reactBtnStyle}
                                                    onMouseOver={(e) => {e.currentTarget.style.background = '#4F46E5'; e.currentTarget.style.boxShadow = '0 0 10px #8B5CF6';}}
                                                    onMouseOut={(e) => {e.currentTarget.style.background = 'rgba(255, 255, 255, 0.05)'; e.currentTarget.style.boxShadow = 'none';}}
                                                >
