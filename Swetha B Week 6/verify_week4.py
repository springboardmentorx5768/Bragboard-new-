...
def test_week4():
    ...
    # Backend expects Form data for simple fields if defined with Form(), or we need to align.
    # checking shoutouts.py: create_shoutout uses Form(...)
    # So we must send data, not json.
    # And for list of recipients, we need to pass multiple values for the same key or weird format.
    # Requests handles this if we pass a list of tuples or just list in data?
    # Actually for recipient_ids which is list[str], we might need to be careful.
    # customized for verify script:
    shoutout_form = {
        "message": "Great job on the deployment!",
    }
    # requests data parameter handles lists by sending multiple keys if list is passed?
    # To be safe with FastAPI, usually we might need `recipient_ids`: [id1, id2]
    
    # We need to construct the payload for data=...
    # For a single recipient:
    
    # The script uses [id_b].
    # Let's try passing list directly in data.
    res_post = requests.post(f"{BASE_URL}/shoutouts/", data={"message": "Great job!", "recipient_ids": [id_b]}, headers=headers_a)
