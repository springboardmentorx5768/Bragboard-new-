...
import { FaTrash, FaHeart, FaSignLanguage as FaHandsClapping, FaLaugh, FaPencilAlt, FaChevronDown, FaChevronUp, FaEllipsisV } from "react-icons/fa";

const CommentItem = ({ comment, shoutoutId, onReply, onUpdate, onDelete, currentUser, isReply = false, parentName = null }) => {
      ...
      <div className="flex gap-3 text-sm">
        ...
        <div className="w-9 h-9 bg-gray-100 rounded flex items-center justify-center font-bold text-xs shrink-0 overflow-hidden shadow-sm border border-gray-200">
          {comment.user && comment.user.profile_image_url ? (
            <img
              src={`http://localhost:8000${comment.user.profile_image_url}`}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 mt-1 text-[11px] text-gray-400">
            ...
            {currentUser && currentUser.id === comment.user_id && !isEditing && (
...
const ShoutOutFeed = ({ refreshTrigger }) => {
  ...
  const handlePostComment = async (shoutoutId, content = null, parentId = null) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/shoutouts/${shoutoutId}/comments`, {
        method: 'POST',
        headers: {
                ...
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-green-100 text-green-600 flex items-center justify-center font-bold text-lg shadow-sm overflow-hidden">
                    {shoutout.sender.profile_image_url ? (
                      <img
                        src={`http://localhost:8000${shoutout.sender.profile_image_url}`}
                        alt={shoutout.sender.name}
                        className="w-full h-full object-cover cursor-zoom-in"
                        onClick={() => setSelectedImage(`http://localhost:8000${shoutout.sender.profile_image_url}`)}
                      />
                    ) : (
