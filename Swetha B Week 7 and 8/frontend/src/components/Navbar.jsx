...
import { FaBars, FaUserCircle, FaPencilAlt, FaSignOutAlt, FaBell, FaChartLine, FaUser, FaTrophy, FaShieldAlt } from "react-icons/fa";
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 text-green-900 hover:text-fuchsia-600 transition-colors"
            >
              <div className="relative">
                <FaBell size={22} />
                {unreadCount > 0 && (
                        ...
                        >
                          ...
                          <div>
                            ...
                            <span className="text-xs text-gray-400 mt-1 block">
                              {new Date(notif.created_at + (notif.created_at.endsWith('Z') ? '' : 'Z')).toLocaleString([], {
                                day: '2-digit',
                                month: '2-digit',
                                year: 'numeric',
                                hour: '2-digit',
          ...
          <button onClick={() => navigate('/dashboard')} className="flex items-center gap-2 text-green-900 hover:text-lime-500 font-medium transition-colors">
            <FaChartLine size={20} />
            <span>Activity</span>
          </button>

          <button onClick={() => navigate('/leaderboard')} className="flex items-center gap-2 text-green-900 hover:text-yellow-600 font-medium transition-colors">
            <FaTrophy size={18} />
            <span>Leaderboard</span>
          </button>

          {currentUser && currentUser.role === 'admin' && (
            <button onClick={() => navigate('/admin')} className="flex items-center gap-2 text-green-900 hover:text-rose-500 font-medium transition-colors">
              <FaShieldAlt size={18} />
              <span>Admin</span>
            </button>
          )}
          ...
          <button onClick={() => navigate('/profile')} className="flex items-center gap-2 text-green-900 hover:text-blue-600 font-medium transition-colors">
            {currentUser && currentUser.profile_image_url ? (
              <img src={`${API_BASE_URL}${currentUser.profile_image_url}`} alt="Profile" className="w-8 h-8 rounded-full object-cover border border-gray-200" />
            ) : (
          ...
          <button onClick={() => navigate('/profile/edit')} className="flex items-center gap-2 text-green-900 hover:text-teal-600 font-medium transition-colors">
            <FaPencilAlt size={18} />
            <span>Edit</span>
          </button>
