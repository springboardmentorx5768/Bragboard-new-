...
def delete_shoutout(
    ...
    if shoutout.sender_id != current_user.id and current_user.role != "admin" and current_user.role != models.UserRole.ADMIN:
...
def create_comment(
    ...
    # Eager load recipients to ensure they are available for notification logic after commit
    shoutout = db.query(models.ShoutOut).options(joinedload(models.ShoutOut.recipients)).filter(models.ShoutOut.id == shoutout_id).first()
    if not shoutout:
        raise HTTPException(status_code=404, detail="Shoutout not found")
    ...
    # Notify shoutout RECIPIENTS (if not self)
    # Get all recipients
    for recipient_rel in shoutout.recipients:
        recipient_id = recipient_rel.recipient_id
        if recipient_id != current_user.id and recipient_id != shoutout.sender_id:
             notification = models.Notification(
                user_id=recipient_id,
                actor_id=current_user.id,
                shoutout_id=shoutout.id,
                type='comment',
                message=f"{current_user.name} commented on a shoutout you received",
            )
             db.add(notification)
...
def report_shoutout(
    ...
    db.add(new_report)
    db.commit()
    return {"message": "Report submitted submitted"}
