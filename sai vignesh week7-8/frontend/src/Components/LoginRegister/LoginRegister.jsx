...
import { FaUser, FaLock, FaEnvelope, FaSignInAlt, FaBriefcase, FaPaperPlane, FaThLarge, FaUsers, FaFilter, FaCheckCircle, FaCommentAlt, FaReply, FaTrophy, FaTrash, FaExclamationTriangle } from "react-icons/fa"; 
...
const Dashboard = ({ userName, userDepartment, userRole }) => {
    ...
    const fetchDashboardData = async () => {
        ...
        try {
            ...
            const resLead = await fetch(`${API_BASE}/shoutouts/stats/leaderboard`, { 
                headers: { "Authorization": `Bearer ${token}` } 
            });
            const dataLead = await resLead.json();
            setLeaderboard(Array.isArray(dataLead) ? dataLead : []);
    ...
    const handleReportPost = async (id) => {
        ...
        if (!reason) return;
        const token = localStorage.getItem("access_token");
        await fetch(`${API_BASE}/shoutouts/${id}/report?reason=${encodeURIComponent(reason)}`, {
            method: "POST", headers: { "Authorization": `Bearer ${token}` }
        });
        setActionMsg("Reported to Admin");
    ...
    const handleDeletePost = async (id) => {
        ...
        const token = localStorage.getItem("access_token");
        await fetch(`${API_BASE}/shoutouts/${id}`, {
        ...
        <div className="dashboard-fixed-layout">
            ...
            <aside className="sidebar-section">
                ...
                <nav className="sidebar-menu-nav">
                    ...
                    {}
                    {userRole === "ADMIN" && (
                        <button className={`nav-btn-item ${view === "admin" ? "active" : ""}`} onClick={() => setView("admin")}><FaFilter className="mr-2" /> Admin Panel</button>
                    )}
                    <button className={`nav-btn-item ${view === "leaderboard" ? "active" : ""}`} onClick={() => setView("leaderboard")}><FaTrophy className="mr-2" /> Leaderboard</button>
                    ...
                    <button className={`nav-btn-ite ${view === "profile" ? "active" : ""}`} onClick={() => setView("profile")}><FaUser className="mr-2" /> My Profile</button>
                </nav>
            ...
            <main className="dashboard-main-content">
                ...
                <div className="scrollable-feed-container">
                    <div className="feed-inner-content">
                                        ...
                                        <div key={s.id} className="shoutout-item" style={{ padding: '20px', marginBottom: '20px' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                                                ...
                                                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                                                    {}
                                                    <FaExclamationTriangle onClick={() => handleReportPost(s.id)} style={{color: '#64748b', cursor:'pointer'}} title="Report" />
                                                    {userRole === "ADMIN" && <FaTrash onClick={() => handleDeletePost(s.id)} style={{color: '#ff4d4d', cursor:'pointer'}} title="Delete" />}
                        ...
                        {}
                        {view === "admin" && (
                            <div className="shoutout-item" style={{ padding: '15px', textAlign: 'center' }}>
                                <h2 className="section-header-text">Admin Panel</h2>
                                <button onClick={() => window.open(`${API_BASE}/shoutouts/export/csv?token=${localStorage.getItem("access_token")}`, "_blank")} className="form-btn" style={{width:'auto', padding:'10px 30px', marginTop:'20px'}}>Export System CSV</button>
                            </div>
                        )}
                        {view === "leaderboard" && (
                            <div className="shoutout-item" style={{ padding: '30px' }}>
                                <h2 className="section-header-text"><FaTrophy color="gold" /> Top Contributors</h2>
                                {leaderboard.map((user, index) => (
                                    <div key={index} style={{ display: 'flex', justifyContent: 'space-between', padding: '30px', borderBottom: '1px solid #ffffff11' }}>
                                        <span style={{ color: 'white' }}>{index + 1}. {user.name}</span>
                                        <span style={{ color: '#5a5761', fontWeight: 'bold' }}>{user.points} Points</span>
                                    </div>
                                ))}
                            </div>
                        )}
                        {/* {view === "admin" && (
                            <div className="shoutout-item" style={{ padding: '30px', textAlign: 'center' }}>
                                <h2 className="section-header-text">Admin Panel</h2>
                                <button onClick={() => window.open(`${API_BASE}/shoutouts/export/csv?token=${localStorage.getItem("access_token")}`, "_blank")} className="form-btn" style={{width:'auto', padding:'10px 30px', marginTop:'20px'}}>Export System CSV</button>
                            </div>
                        )} */}
