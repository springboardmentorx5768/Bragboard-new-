...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        setBragsCount(Math.floor(Math.random() * 50) + 5);
  ...
  const handleCreateBrag = async () => {
    ...
    try {
      ...
      const response = await api.post("/brags", formData, {
        headers: {
                ...
                <div className="stats-grid">
                  ...
                  >
                    ...
                    <div className="stat-content">
                      ...
                      <p className="stat-number">{bragsCount * 3}</p>
                    </div>
                  </motion.div>
                    ...
                    >
                      <div className="brag-header">
                        ...
                        <div className="brag-meta">
                          <div className="brag-author">{brag.author.name}</div>
                  ...
                  <div className="filters-grid">
                    <div className="filter-group">
                      ...
                      >
                        ...
                        {[...new Set(departmentMembers.map(m => m.department).filter(Boolean))].map(dept => (
                    ...
                    >
                      <div className="feed-brag-header">
                        ...
                        <div className="feed-brag-meta">
                          <div className="feed-brag-author">{brag.author.name}</div>
                      ...
                      {brag.attachments && brag.attachments.length > 0 && (
                        <div className="feed-brag-attachments">
                          {brag.attachments.map((attachment) => (
                            <div key={attachment.id} className="feed-brag-attachment">
                              {attachment.content_type.startsWith('image/') ? (
                                <img
                                  src={`http://localhost:8000/uploads/${attachment.filename}`}
                                  alt={attachment.original_filename}
                                  className="feed-brag-attachment-image"
                                  onError={(e) => {
                                    e.target.style.display = 'none';
                                  }}
                                />
                              ) : (
                                <div className="feed-brag-attachment-file">
                                  <span className="file-icon">📎</span>
                                  <span className="file-name">{attachment.original_filename}</span>
                                </div>
                              )}
