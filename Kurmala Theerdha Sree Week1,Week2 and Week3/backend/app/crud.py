...
def get_department_stats(db: Session, department: str):
    ...
    members = get_department_members(db, department)

    total_members = len(members)
    admin_count = sum(1 for m in members if m.role == "admin")
    employee_count = total_members - admin_count
    ...
    return {
        ...
        "total_brags": total_members * 5  # Placeholder logic
    }


def get_department_activity(db: Session, department: str, limit: int = 10):
    """Get recent activity from department members"""
    members = get_department_members(db, department)
    ...
    for member in members[:limit]:
        activities.append({
            ...
            "description": f"{member.name} joined {member.department} department"
        })
...
def get_department_stats(db: Session, department: str):
    """Get statistics for a department"""
    total_members = db.query(models.User).filter(models.User.department == department).count()
